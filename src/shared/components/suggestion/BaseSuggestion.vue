...
const filteredItems = computed(() => {
    ...
    if (keyword.length < props.minChars || !isDropdownOpen.value || isLoading.value) return [];
...
const showSuggestions = computed(() => {
    return props.modelValue.trim().length >= props.minChars && isDropdownOpen.value && !isLoading.value;
});
...
const outsideClick = (e) => {
    if (!wrapper.value?.contains(e.target) && !e.target.closest('.form-control')) {
        isDropdownOpen.value = false;
        selectedIndex.value = -1;
    } else if (e.target.closest('.form-control')) {
        isDropdownOpen.value = true;
    }
};
            ...
            <table class="table table-striped table-bordered table-hover m-0">
                ...
                <tbody>
                    <tr v-if="filteredItems.length == 0">
                        <td :colspan="headers.length" class="text-center">No Result Found</td>
                    </tr>
                    <tr
...
.child-circle-loader div:nth-child(1) { top: 0px; left: 8px; animation-delay: 0s; }
.child-circle-loader div:nth-child(2) { top: 2px; left: 14px; animation-delay: -0.15s; }
.child-circle-loader div:nth-child(3) { top: 8px; left: 16px; animation-delay: -0.3s; }
.child-circle-loader div:nth-child(4) { top: 14px; left: 14px; animation-delay: -0.45s; }
.child-circle-loader div:nth-child(5) { top: 16px; left: 8px; animation-delay: -0.6s; }
.child-circle-loader div:nth-child(6) { top: 14px; left: 2px; animation-delay: -0.75s; }
.child-circle-loader div:nth-child(7) { top: 8px; left: 0px; animation-delay: -0.9s; }
.child-circle-loader div:nth-child(8) { top: 2px; left: 2px; animation-delay: -1.05s; }
